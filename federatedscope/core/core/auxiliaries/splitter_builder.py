...
def get_splitter(config):
    ...
    # graph splitter
        ...
        if config.data.type=='ms_academic':
            if config.federate.client_num==10:
                splitter = LouvainSplitter(client_num, delta=500)
            else:
                splitter = LouvainSplitter(client_num, delta=200)
        else:
