...
from federatedscope.gfl.feddep.dec_cluster.ptdec.utils import target_distribution
...
def train(
    ...
    predicted_previous = torch.tensor(np.copy(predicted),
                                      dtype=torch.long)  # [0,1,2,4,2,4,...]
    ...
    for epoch in range(epochs):
        ...
        predicted = predict(dataset,
                            ...
                            cuda=cuda)
        delta_label = (float(
            (predicted != predicted_previous).float().sum().item()) /
