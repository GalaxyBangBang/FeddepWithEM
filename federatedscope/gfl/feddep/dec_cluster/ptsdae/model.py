...
from federatedscope.gfl.feddep.dec_cluster.ptsdae.dae import DenoisingAutoencoder
    ...
    for epoch in range(epochs):
        ...
        for index, batch in enumerate(data_iterator):
            if (isinstance(batch, tuple) or isinstance(batch, list)):
                batch = batch[
                    0]  # if we have a prediction label, strip it away
        ...
        if update_freq is not None and epoch % update_freq == 0:
            if validation_loader is not None:
                ...
                validation_actual = torch.cat(
                    [val_batch for val_batch in validation_loader])
                if cuda:
...
def pretrain(
    ...
    for index in range(number_of_subautoencoders):
        ...
        sub_autoencoder = DenoisingAutoencoder(
            ...
            activation=torch.nn.ReLU() if index !=
            (number_of_subautoencoders - 1) else None,
