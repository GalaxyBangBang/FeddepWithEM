...
class SDAETransformerBase(TransformerMixin, BaseEstimator):
    def __init__(
        ...
        optimiser_pretrain: Callable[[torch.nn.Module],
                                     torch.optim.Optimizer] = lambda x: SGD(
                                         x.parameters(), lr=0.1, momentum=0.9),
        optimiser_train: Callable[[torch.nn.Module],
                                  torch.optim.Optimizer] = lambda x: SGD(
                                      x.parameters(), lr=0.1, momentum=0.9),
        scheduler: Optional[
            Callable[[torch.optim.Optimizer],
                     Any]] = lambda x: StepLR(x, 100, gamma=0.1),
...
class SDAERepresentationTransformer(SDAETransformerBase):
    def transform(self, X):
        ...
        return np.concatenate(
            [
                torch.cat(x).numpy()
                for x in features_encoder + features_decoder[:-1]
            ],
            axis=1,
        )
