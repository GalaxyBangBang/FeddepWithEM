...
from federatedscope.gfl.model.sage import SAGE_Net, Encoder, Classifier_F
    ...
    if model_config.task.startswith('node'):
        if model_config.type == 'gcn':
            ...
            model = SAGE_Net(x_shape[-1],
                             ...
                             max_depth=model_config.layer,
                             dropout=model_config.dropout)
        elif model_config.type == 'feddep_encoder':
            model = Encoder(x_shape[-1],
                            model_config.out_channels,
                            hidden=model_config.hidden,
                            max_depth=model_config.layer,
                            dropout=model_config.dropout)
        elif model_config.type == 'feddep_f':
            model = Classifier_F((x_shape[-1], model_config.in_channels),
                                 model_config.out_channels,
                                 hidden=model_config.hidden,
                                 max_depth=model_config.layer,
                                 dropout=model_config.dropout)
