...
from federatedscope.gfl.model.myfedsageplus import LocalSage_Plus, FedSage_Plus
from federatedscope.gfl.myfedsageplus.utils import GraphMender, HideGraph
from federatedscope.gfl.myfedsageplus.trainer import LocalGenTrainer, \
    FedGenTrainer
...
class FedSagePlusServer(Server):
    def __init__(self,
        ...
        self.fedgen_epoch = 2 * self._cfg.myfedsageplus.fedgen_epoch
    ...
    def check_and_move_on(self, check_eval_result=False):
        ...
        ) and self.state < self._cfg.myfedsageplus.fedgen_epoch and self.state\
                % 2 == 0:
        ...
        if self.check_buffer(
                self.state, self.client_num
        ) and self.state == self._cfg.myfedsageplus.fedgen_epoch:
            self.state += 1
...
class FedSagePlusClient(Client):
    def __init__(self,
        ...
        self.data = data
        self.hide_data = HideGraph(self._cfg.myfedsageplus.hide_portion)(
            data['data'])
        ...
        self.gen = LocalSage_Plus(
            data['data'].x.shape[-1],
            self._cfg.model.out_channels,
            hidden=self._cfg.model.hidden,
            gen_hidden=self._cfg.myfedsageplus.gen_hidden,
            dropout=self._cfg.model.dropout,
            num_pred=self._cfg.myfedsageplus.num_pred)
        self.clf = model
    ...
    def callback_funcs_for_local_pre_train(self, message: Message):
        ...
        for i in range(self._cfg.myfedsageplus.loc_epoch):
