    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        if ctx.cur_split == 'train':
            ...
            if self.is_NeighborSampler:
                ...
                if isinstance(ctx.model, Classifier_F):
                    if "mend_emb" not in ctx.data['data']:
                        mend_emb = torch.zeros((len(ctx.data['data'].x),
                                                ctx.model.emb_len)).to(device)
                    else:
                        mend_emb = ctx.data['data'].mend_emb
                    pred = ctx.model(
                        (ctx.data['data'].x[n_id], mend_emb[n_id]), adjs=adjs)
                else:
                ...
                batch = ctx.data_batch.to(ctx.device)
                if isinstance(ctx.model, Classifier_F):
                    if "mend_emb" not in batch:
                        mend_emb = torch.zeros(
                            (len(batch.x), ctx.model.emb_len)).to(device)
                    else:
                        mend_emb = batch.mend_emb
                    pred = ctx.model(batch.x, mend_emb,
            ...
            if isinstance(ctx.model, Classifier_F):
                if "mend_emb" not in ctx.data['data']:
                    ...
                    mend_emb = ctx.data['data'].mend_emb
                pred = ctx.model.inference(ctx.data['data'].x, mend_emb,
                                           subgraph_loader, ctx.device)[mask]
            else:
